...
while true
    ...
    if not(j==181) || not(i==181) 
        if (i>1 && i<=nrows)    update (i-1,j,min_dist+1,current);      end
        if (i<=1)               update (nrows,j,min_dist+1,current);    end    
        if (i>=1 && i<nrows)    update (i+1,j,min_dist+1,current);      end
        if (i >= nrows)         update (1,j,min_dist+1,current);        end
        if (j>1 && j<=ncols)    update (i,j-1,min_dist+1,current);      end
        if (j<=1)               update (i,ncols,min_dist+1,current);    end    
        if (j>=1 && j<ncols)    update (i,j+1,min_dist+1,current);      end
        if (j>=ncols)           update (i,j,min_dist+1,current);        end
    end
end    
